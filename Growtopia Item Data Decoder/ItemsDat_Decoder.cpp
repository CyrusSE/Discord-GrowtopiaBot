...
auto read_items(std::uint8_t* _Data) noexcept -> void
{
	...
	std::string result2;
	std::string result3;
	std::string rillresult;
	std::string rillresult2;
	std::string rillresult3;
	...
	for (int i = 0; i < itemsize; i++)
	{
		...
		// std::cout << "Item ID: " << it._M_Id << std::endl;
		// std::cout << "Name: " << it._M_Name << std::endl;
		//result += "Name: " + it._M_Name + "\nID: " + std::to_string(it._M_Id) + "\n-----------------------------------\n";
		result3 += "Name: " + it._M_Name + "\nID: " + std::to_string(it._M_Id) + "\nRarity: " + std::to_string(it._M_Rarity) + "\nMax Amount: " + std::to_string(it._M_Max_Amount) + "\nGrow Time: " + std::to_string(it._M_Grow_Time) + "\nVolume: " + std::to_string(it._M_Audio_Volume) + "\nCategory: " + std::to_string(it._M_Item_Category) + "\nTexture Path: " + it._M_Texture_Path + "\n-----------------------------------\n";
		if (it._M_Id == 0){
		 continue;
		}
		result += ",\n{\"id\":" + std::to_string(it._M_Id) + ",\"name\":\"" + it._M_Name + "\"}";
		result2 += ",\n\"" + std::to_string(it._M_Id) + "\":\"" + it._M_Name + "\"";
		// std::cout << "Texture Path: " << it._M_Texture_Path << std::endl;
		// std::cout << "Category: " << static_cast<int>(it._M_Item_Category) << std::endl;
		// std::cout << "Rarity: " << it._M_Rarity << std::endl;
		// std::cout << "Max Amount: " << static_cast<int>(it._M_Max_Amount) << std::endl;
		// std::cout << "Grow Time: " << it._M_Grow_Time << std::endl;
		// // std::cout << "Ingredient: " << it._M_Pet_Ability << std::endl; //volume
		// std::cout << "-----------------------------------" << std::endl;
		//make all result to one string like use +=
		items.push_back(std::move(it));
		//result += "Name: " + it._M_Name + "\n ID: " + std::to_string(it._M_Id) + "\n-----------------------------------\n";
	}
	rillresult = "{\"itemid\":[{\"id\":0,\"name\":\"Blank\"}" + result + "]\n}";
	rillresult2 = "{\n\"0\":\"Blank\"" + result2 + "\n}";
	// std::cout << result;
	// //post the result to txt file
	std::ofstream file("itemsid.json");
    file << rillresult;
    file.close();
	std::ofstream file2("itemsid2.json");
    file2 << rillresult2;;
    file2.close();
	std::ofstream file3("itemsid3.txt");
    file3 << result3;;
    file3.close();
